...
if st.session_state.gdf_master is not None:
    ...
    if st.session_state.analysis_done and st.session_state.gangguan_coords:
        ...
        if st.session_state.gdf_nearby is not None:
            ...
            # Jika user mengklik peta, jalankan filter 5km dari titik klik dan tampilkan fitur "alpro"
            last_click = None
            if isinstance(map_data, dict):
                last_click = map_data.get('last_clicked') or map_data.get('clicked')

            if last_click:
                click_lat = last_click.get('lat')
                click_lon = last_click.get('lng') or last_click.get('lon')
                if click_lat is not None and click_lon is not None:
                    st.info(f"Klik terdeteksi: {click_lat:.6f}, {click_lon:.6f} — mencari 'alpro' dalam radius 5 km")
                    # buat point (lon, lat)
                    p = Point(click_lon, click_lat)
                    nearby5 = filter_features_nearby(st.session_state.gdf_master, p, radius_km=5)

                    # cari fitur yang mengandung substring 'alpro' di salah satu atribut
                    def find_alpro(gdf):
                        rows = []
                        for idx, row in gdf.iterrows():
                            for col in gdf.columns:
                                ...
                                val = row[col]
                                try:
                                    if isinstance(val, str) and 'alpro' in val.lower():
                                        rows.append(row)
                                        break
                                except Exception:
                                    continue
                    ...
                    alpro_df = find_alpro(nearby5)
                    st.success(f"Ditemukan {len(alpro_df)} fitur 'alpro' dalam 5 km dari titik klik")
                    if not alpro_df.empty:
                        st.dataframe(alpro_df.drop(columns='geometry', errors='ignore'))
                        # tampilkan peta fokus
                        m_click = create_map(alpro_df, [click_lat, click_lon], zoom=13)
                        st_folium(m_click, width=800, height=400, key=f"click_map_{click_lat}_{click_lon}")
